...
async function saveToDb(event) {
  ...
  try{
    const res = await axios.post("http://localhost:4000/admin/addNewExpense", obj);
      ...
      //alert div auto hide after 3sec
      ...
      //document.body.innerHTML += `<h4 class="bg-red-400 text-white text-bold"> Server error ${err} , while saving data.  Please try after some time</h4>`;
    };
}
...
async function showexpenses(expenseItem) {
  ...
  document.querySelector('#error-alert').classList.toggle("hidden")
  //document.body.innerHTML += `<h4> Server error ${err} , while printing data to screen. Please try after some time</h4>`;
   }
}
...
async function updateUser(expId) {
  ...
  try{
    const res1 = await axios.put(`http://localhost:4000/admin/updateExpense/${expId}`, obj);
      ...
      document.querySelector('#error-alert').classList.toggle("hidden")
      //document.body.innerHTML += `<h4> Server error ${err} , while updating data. Please try after some time</h4>`;
    }
  try{
...
async function removeExpenseFromScreen(expId) {
  try {
    ...
    document.querySelector('#error-alert').classList.toggle("hidden")
    //document.body.innerHTML += `<h4> Server error ${err} , while removing data from screen. Please try after some time</h4>`;
  }
}
...
async function deleteexpense(expId) {
  try{
    ...
    removeExpenseFromScreen(expId);    
      ...
      document.querySelector('#error-alert').classList.toggle("hidden")
      // document.body.innerHTML += `<h4>${err} ,in deleting. Please Refresh the page(press F5 button) and try to delete again.</h4>`;
    }
}
